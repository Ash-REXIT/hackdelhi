// app/auth/otp.tsx
import { FontAwesome } from "@expo/vector-icons";
import { useLocalSearchParams, useRouter } from "expo-router";
import React, { useState } from "react";
import {
    Alert,
    Text,
    TextInput,
    TouchableOpacity,
    View,
} from "react-native";
import { KeyboardAwareScrollView } from "react-native-keyboard-aware-scroll-view";

type Params = {
  phone?: string;
  otp?: string;
  mode?: string; // "signup" | "login"
};

export default function Otp() {
  const router = useRouter();
  const { phone, otp, mode } = useLocalSearchParams<Params>();
  const [code, setCode] = useState("");

  const handleVerify = () => {
    if (!code) {
      Alert.alert("Enter OTP", "Please enter the OTP sent to your phone.");
      return;
    }

    if (code !== otp) {
      Alert.alert("Incorrect OTP", "The code you entered is not correct.");
      return;
    }

    // ✅ OTP correct
    if (mode === "signup") {
      // after signup → go to user details
      router.push("/auth/user-details" as any);
    } else {
      // after login – for now also go to user details (you can change to /home later)
      router.push("/auth/user-details" as any);
    }
  };

  return (
    <KeyboardAwareScrollView
      contentContainerStyle={{
        flexGrow: 1,
        justifyContent: "center",
        paddingHorizontal: 24,
        paddingVertical: 15,
      }}
      enableOnAndroid={true}
      extraScrollHeight={30}
      keyboardOpeningTime={0}
    >
      <View>
        <View className="items-center mb-6">
          <FontAwesome name="paper-plane" size={48} color="#0D1821" />
          <Text className="text-3xl mt-8 font-bold text-textmain">
            Enter OTP
          </Text>
        </View>

        {phone && (
          <Text className="text-center text-textmuted mb-4">
            We sent a code to {phone}
          </Text>
        )}

        <Text className="text-textmuted font-medium">OTP</Text>
        <TextInput
          className="bg-[#c7c7c7] p-3 rounded-xl mt-1 mb-3 text-center tracking-[8px]"
          placeholder="000000"
          keyboardType="number-pad"
          maxLength={6}
          value={code}
          onChangeText={setCode}
        />

        <TouchableOpacity
          className="bg-accent p-4 rounded-xl mt-4"
          onPress={handleVerify}
        >
          <Text className="text-center text-white font-bold text-lg">
            Verify
          </Text>
        </TouchableOpacity>
      </View>
    </KeyboardAwareScrollView>
  );
}
